...
//                           ... et {processedData.length - 5} autre(s)
//                           enregistrement(s)
//                         </div>
//                       )}
//                     </div>
...
const ExcelProcessorApp: React.FC = () => {
  ...
  // États pour les options CSV
  const [showCsvOptionsModal, setShowCsvOptionsModal] = useState<boolean>(false);
  const [availableSheets, setAvailableSheets] = useState<string[]>([]);
  const [csvOption, setCsvOption] = useState<'first' | 'specific' | 'combine'>('first');
  const [selectedSheet, setSelectedSheet] = useState<string>('');
  ...
  const filteredHistory = history.filter((entry) => {
    const matchesText =
      searchText === "" ||
      entry.original_filename.toLowerCase().includes(searchText.toLowerCase()) ||
      entry.file_id.toLowerCase().includes(searchText.toLowerCase()) ||
      (entry.processed_filename &&
        entry.processed_filename.toLowerCase().includes(searchText.toLowerCase()));
  ...
  const processFile = async (): Promise<void> => {
    ...
    try {
      ...
      if (error instanceof ApiError) {
        ...
        setValidationErrors(["Erreur inattendue lors du traitement du fichier"]);
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      <div className="container mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow-xl overflow-hidden">
          <div className="bg-gradient-to-r from-green-600 to-green-600 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-blue-100 mt-2">
                  Génération automatique d'identifiants et validation des données
                </p>
              </div>
              <div className="flex items-center gap-2">
                <div
          ...
          <div className="p-8">
            {activeTab === "upload" && (
              <div className="space-y-6">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <ul className="text-sm text-blue-800 space-y-1">
                    <li>• Colonnes obligatoires : <strong>nom</strong>, <strong>prenom</strong>, <strong>numero</strong></li>
                    <li>• Le système génèrera automatiquement : <strong>email</strong> et <strong>mot_de_passe</strong></li>
                    <li>• Format de numéro : 8 à 15 chiffres (avec ou sans indicatif)</li>
                  </ul>
                </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                      <div className="flex items-start gap-3">
                        <CheckCircle className="w-6 h-6 text-green-600 flex-shrink-0" />
                        <div>
                          <h3 className="font-semibold text-green-900">Traitement réussi !</h3>
                          <p className="text-green-800 mt-1">
                            {processedData.length} enregistrement(s) traité(s) avec succès.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                      <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
                        <h3 className="font-semibold text-gray-900">Aperçu des données traitées</h3>
                      </div>

                      {/* Navigation entre les feuilles si plusieurs feuilles */}
                      {(() => {
                        const uniqueSheets = [...new Set(processedData.map(d => d.sheet_name))];
                        if (uniqueSheets.length > 1) {
                          return (
                            <div className="bg-gray-100 border-b border-gray-200 px-4 py-3">
                              <div className="flex items-center gap-2 overflow-x-auto">
                                <span className="text-sm font-medium text-gray-700 mr-2">Feuilles:</span>
                        ...
                        <table className="w-full">
                          <thead className="bg-gray-100">
                            <tr>
                              ...
                              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Mot de passe</th>
                            </tr>
                          </thead>
                          <tbody>
                            {(() => {
                              ...
                              if (uniqueSheets.length > 1) {
                                ...
                                displayData = processedData.filter(d => d.sheet_name === currentSheet);
                              }
                              
                              return displayData.slice(0, 10).map((row, index) => (
                                <tr key={index} className="border-t border-gray-200">
                                  ...
                                  <td className="px-4 py-3 text-sm text-gray-800">{row.nom}</td>
                                  <td className="px-4 py-3 text-sm text-gray-800">{row.prenom}</td>
                                  <td className="px-4 py-3 text-sm text-gray-800">{row.numero}</td>
                                  <td className="px-4 py-3 text-sm text-blue-600">{row.email}</td>
                                  <td className="px-4 py-3 text-sm font-mono text-gray-600">{row.mot_de_passe}</td>
                                </tr>
                              ));
                            })()}
                          </tbody>
                      ...
                      {(() => {
                        ...
                        if (displayData.length > 10) {
                          return (
                            <div className="bg-gray-50 px-4 py-3 border-t border-gray-200 text-sm text-gray-600 text-center">
                              ... et {displayData.length - 10} autre(s) enregistrement(s) dans cette feuille
                            </div>
                          );
                        }
                        return null;
            ...
            {activeTab === "history" && (
              <div className="space-y-4">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-gray-900">Historique des traitements</h2>
                  <button
                    onClick={loadHistory}
                    disabled={loadingHistory}
                    className="px-4 py-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors flex items-center gap-2 disabled:opacity-50"
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <h3 className="font-semibold text-gray-900 mb-3">Filtres de recherche</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Rechercher par nom de fichier
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4 flex-1">
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <h3 className="font-semibold text-blue-600">{entry.original_filename}</h3>
                                  <ArrowBigRightDash />
                                  <h3 className="font-semibold text-green-600">{entry.processed_filename}</h3>
                                </div>
                                <p className="text-sm text-gray-600">
                                  {new Date(entry.upload_date).toLocaleString("fr-FR")} •{" "}
                                  {entry.record_count} enregistrement(s)
                                </p>
                                {entry.error_message && (
                                  <p className="text-sm text-red-600 mt-1">Erreur: {entry.error_message}</p>
                                )}
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                              ...
                              {entry.status === "success" && entry.processed_filename && (
                                <button
                                  onClick={() => openFormatModal(entry.file_id)}
                                  className="p-2 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors"
                                  title="Télécharger"
                                >
                                  <Download className="w-5 h-5" />
        ...
        <div className="mt-8 text-center text-gray-600 text-sm">
          <p>Application de traitement automatisé • Connecté au serveur backend</p>
        </div>
      </div>
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-md w-full p-6">
            <h3 className="text-xl font-bold text-gray-900 mb-4">Nom du fichier à traiter</h3>
            <p className="text-sm text-gray-600 mb-4">
              Donnez un nom à ce fichier qui sera enregistré dans le système
            </p>
            <div className="mb-6">
      ...
      {showFormatModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full p-6">
            ...
            <p className="text-sm text-gray-600 mb-6">
              Sélectionnez le format dans lequel vous souhaitez télécharger votre fichier traité
            </p>
            ...
            {selectedFormat === "csv" && availableSheets.length > 1 && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                <div className="flex items-start gap-2">
                  <AlertCircle className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
                  <div className="text-sm text-blue-800">
                    <strong>Info:</strong> Ce fichier contient {availableSheets.length} feuilles. 
                    Vous pourrez choisir comment les exporter après avoir cliqué sur Télécharger.
                  </div>
                </div>
              </div>
            )}
            ...
            {selectedFormat === "xlsb" && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                <div className="flex items-start gap-2">
                  <AlertCircle className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
                  <div className="text-sm text-blue-800">
                    <strong>Info:</strong> Le format XLSB sera téléchargé en XLSX (compatibilité).
                  </div>
                </div>
              </div>
            )}
