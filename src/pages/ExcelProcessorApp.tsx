...
const ExcelProcessorApp: React.FC = () => {
  ...
  // États pour la pagination par feuille
  const [currentPageBySheet, setCurrentPageBySheet] = useState<
  ...
  // Réinitialiser la pagination quand on change de feuille
  useEffect(() => {
    if (processedData && currentSheetIndex >= 0) {
      const uniqueSheets = [...new Set(processedData.map((d) => d.sheet_name))];
      const currentSheet = uniqueSheets[currentSheetIndex];
      if (currentSheet) {
        setCurrentPageBySheet((prev) => ({
          ...prev,
          [currentSheet]: 1,
        }));
      }
    }
  }, [currentSheetIndex, processedData]);
  ...
  const processFile = async (): Promise<void> => {
    ...
    try {
      ...
      // Stocker les noms de feuilles et l'ID du fichier
      setSheetNames(result.sheet_names || []);
      setCurrentSheetIndex(0);
      setCurrentPageBySheet({}); // Réinitialiser la pagination

      // Récupérer toutes les données du fichier traité
      const fileDetails = await apiService.getFileDetails(result.file_id);

      // Mapper les données pour s'assurer que mot_de_passe est rempli
      // Ajouter sheet_name basé sur le cohorte (pour affichage et filtrage)
      const mappedRecords = fileDetails.records.map((record: any) => ({
        ...record,
        prenoms: record.prenom || record.prenoms,
        mot_de_passe: record.password_hash || record.mot_de_passe,
        sheet_name: record.cohorte
          ? record.cohorte.replace(/-/g, " ")
          : "default",
      }));

      setProcessedData(mappedRecords);
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <ul className="text-sm text-blue-800 space-y-1">
                    ...
                    <li>
                      ...
                      <strong>Email</strong>, <strong>course1</strong>,{" "}
                      <strong>role1 (student)</strong> et{" "}
                      <strong>Cohorte1</strong>
                    </li>
                  </ul>
                </div>
                ...
                {validationSuccess && processedData && (
                  <div className="space-y-4">
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                        ...
                        <table className="w-full">
                          ...
                          <tbody>
                            {(() => {
                              ...
                              const currentSheet =
                                sheetNames[currentSheetIndex];

                              // Filtrer par feuille si plusieurs feuilles
                              if (sheetNames.length > 1 && currentSheet) {
                                displayData = processedData.filter(
                                  (d) => d.sheet_name === currentSheet
                                );
                              }

                              // Initialiser la page courante pour cette feuille
                              const sheetKey = currentSheet || "default";
                              const currentPageForSheet =
                                currentPageBySheet[sheetKey] || 1;
                              const startIdx =
                                (currentPageForSheet - 1) * itemsPerSheet;
                              const endIdx = startIdx + itemsPerSheet;
                      ...
                      {/* Pagination */}
                      {(() => {
                        const currentSheet = sheetNames[currentSheetIndex];
                        let displayData = processedData;
                        ...
                        const totalPages = Math.ceil(
                          displayData.length / itemsPerSheet
                        );
                          ...
                          <div className="bg-gray-50 px-4 py-4 border-t border-gray-200">
                            <div className="flex items-center justify-between">
                              <div className="text-sm text-gray-600">
                                Affichage{" "}
                                {(currentPageForSheet - 1) * itemsPerSheet + 1}{" "}
                                à{" "}
                                {Math.min(
                                  currentPageForSheet * itemsPerSheet,
                                  displayData.length
                                )}{" "}
                                sur {displayData.length} enregistrement(s)
                              </div>
                              <div className="flex gap-2">
                                <button
                                  onClick={() => {
                                    if (currentPageForSheet > 1) {
                                      setCurrentPageBySheet({
                                        ...currentPageBySheet,
                                        [sheetKey]: currentPageForSheet - 1,
                                      });
                                    }
                                  }}
                                  disabled={currentPageForSheet === 1}
                                  className="px-3 py-2 text-sm font-medium rounded-lg bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                                >
                                  Précédent
                                </button>
                                      ...
                                      page === "..." ? (
                                        ...
                                        <button
                                          ...
                                          onClick={() => {
                                            setCurrentPageBySheet({
                                              ...currentPageBySheet,
                                              [sheetKey]: page as number,
                                            });
                                          }}
                                          className={`px-3 py-2 text-sm font-medium rounded-lg transition-all ${
                                            page === currentPageForSheet
                                              ? "bg-blue-600 text-white shadow-md"
                                ...
                                <button
                                  onClick={() => {
                                    if (currentPageForSheet < totalPages) {
                                      setCurrentPageBySheet({
                                        ...currentPageBySheet,
                                        [sheetKey]: currentPageForSheet + 1,
                                      });
                                  ...
                                  disabled={currentPageForSheet === totalPages}
                                  className="px-3 py-2 text-sm font-medium rounded-lg bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                                >
                                  Suivant
                                </button>
