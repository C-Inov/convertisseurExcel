...
# MAX_FILE_SIZE = os.getenv("MAX_FILE_SIZE")  # 16MB
    ...
    try:
        ...
        for sheet_name in excel_file.sheet_names:
            ...
            for _, row in df.iterrows():
                email = generate_email(str(row["prenom"]), str(row["nom"]))
                seudo = generate_pseudonyme(
                    str(row["nom"]), str(row["prenom"]), str(row["classe"])
                )
        ...
        for sheet_name, records in sheets_dict.items():
            ...
            # En-têtes avec style
            headers = ["Nom", "Prénom", "seudo", "Email", "Mot de passe", "classe"]
            ...
            for row_num, record in enumerate(records, 2):
                ...
                ws.cell(row=row_num, column=3, value=record["seudo"])
