...
if os.environ.get("FLASK_ENV") == "production":
    ...
    # URL frontend en production
    CORS(app, origins=["https://ensemble.C-inov.com"])
else:
...
def validate_excel_data(df):
    ...
    # ✅ Détecter si la colonne "nom prenom" ou "nom prénom" existe
    combined_cols = [
        col for col in df.columns if "nom prenoms" in col or "nom prénoms" in col
    ]

    if combined_cols:
        col_name = combined_cols[0]  # première occurrence
        # Séparer la colonne en deux (en supposant "Nom Prenom" ou "Nom Prénom")
        df[["nom", "prenoms"]] = df[col_name].astype(str).str.split(n=1, expand=True)
        df.drop(columns=[col_name], inplace=True)

    required_columns = ["nom", "prenoms", "classe"]
    ...
    for index, row in df.iterrows():
        ...
        if pd.isna(row["prenoms"]) or str(row["prenoms"]).strip() == "":
        ...
        if not pd.isna(row["nom"]) and not pd.isna(row["prenoms"]):
            email = generate_email(str(row["prenoms"]), str(row["nom"]))
...
def login():
    ...
    try:
        ...
        if user["deleted"]:
            return jsonify({"message": "Compte supprimé. Accès refusé."}), 403
...
def upload_file():
    ...
    try:
        ...
        for sheet_name in excel_file.sheet_names:
            ...
            # Valider les données de cette feuille - GESTION D'ERREUR AMÉLIORÉE
            try:
                validation_result = validate_excel_data(df)

                # Gérer différents types de retours de validate_excel_data
                if isinstance(validation_result, tuple) and len(validation_result) == 2:
                    is_valid, sheet_errors = validation_result
                elif isinstance(validation_result, bool):
                    is_valid = validation_result
                    sheet_errors = []
                else:
                    # Si le format n'est pas reconnu, considérer comme valide
                    is_valid = True
                    sheet_errors = []

            except Exception as e:
                # En cas d'erreur dans la validation, logger et continuer
                print(
                    f"Erreur lors de la validation de la feuille {sheet_name}: {str(e)}"
                )
                is_valid = False
                sheet_errors = [f"Erreur de validation: {str(e)}"]
            ...
            for _, row in df.iterrows():
                email = generate_email(str(row["prenoms"]), str(row["nom"]))
                pseudo = generate_pseudonyme(
                    str(row["nom"]), str(row["prenoms"]), str(row["classe"])
                )
                ...
                processed_record = {
                    ...
                    "prenoms": str(row["prenoms"]).strip(),
                    "pseudo": pseudo,
                    "email": email,
                    "mot_de_passe": password,
                    "classe": str(row["classe"]).strip(),
                    "student": "student",
                    "cohorte": sheet_name.replace(" ", "-"),
                }
                ...
                if connection:
                    ...
                    cursor.execute(
                        ...
                        (file_id, nom, prenom, pseudo, email, password_hash, classe, student, cohorte)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                    """,
                        (
                            file_id,
                            processed_record["nom"],
                            processed_record["prenoms"],
                            processed_record["pseudo"],
                            processed_record["email"],
                            password_hash,
                            processed_record["classe"],
                            processed_record["student"],
                            processed_record["cohorte"],
                        ),
                    )
                    connection.commit()
                    cursor.close()
        ...
        for sheet_name, records in sheets_dict.items():
            ...
            for row_num, record in enumerate(records, 2):
                ws.cell(row=row_num, column=1, value=record["pseudo"])
                ws.cell(row=row_num, column=2, value=record["mot_de_passe"])
                ws.cell(row=row_num, column=3, value=record["nom"])
                ws.cell(row=row_num, column=4, value=record["prenoms"])
                ws.cell(row=row_num, column=5, value=record["email"])
                ws.cell(row=row_num, column=6, value=record["classe"])
                ws.cell(row=row_num, column=7, value=record["student"])
                ws.cell(row=row_num, column=8, value=record["cohorte"])
...
# @app.route("/api/file/<file_id>/fe", methods=["DELETE"])
...
def delete_file(file_id):
    ...
    try:
        ...
        # Récupérer les noms de fichiers associés à cet ID
        cursor.execute(
            """
            SELECT original_filename, processed_filename
            FROM file_history
        ...
        # Supprimer les fichiers physiques avec gestion d'erreur améliorée
        upload_path = os.path.join(
            app.config.get("UPLOAD_FOLDER", "uploads"),
            f"{file_id}_{result['original_filename']}",
        )
        ...
        if processed_filename:
            processed_path = os.path.join(
                app.config.get("PROCESSED_FOLDER", "processed"), processed_filename
            )

        cursor.close()
        connection.close()

        # Fonction pour supprimer un fichier avec plusieurs tentatives
        def safe_delete_file(file_path, max_attempts=3):
            if not file_path or not os.path.exists(file_path):
                return True, "Fichier non trouvé"
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt < max_attempts - 1:
                        ...
                        return (
                            False,
                            f"Impossible de supprimer le fichier après {max_attempts} tentatives",
                        )
                except Exception as e:
        ...
        upload_success, upload_message = safe_delete_file(upload_path)
        processed_success, processed_message = safe_delete_file(processed_path)

        # Préparer la réponse
        messages = []
        if not upload_success:
            messages.append(f"Fichier upload: {upload_message}")
        if not processed_success:
            messages.append(f"Fichier traité: {processed_message}")

        if messages:
            return (
                jsonify(
                    {
                        "message": "Entrée supprimée de la base de données, mais certains fichiers n'ont pas pu être supprimés",
                        "warnings": messages,
                    }
                ),
                200,
            ...
            return jsonify({"message": "Fichier et entrées supprimés avec succès"}), 200

    except Exception as e:
        import traceback

        print("Erreur lors de la suppression :", traceback.format_exc())
        return (
            jsonify({"error": "Erreur lors de la suppression", "details": str(e)}),
            500,
        )
