    ...
    try:
        ...
        for sheet_name in excel_file.sheet_names:
            ...
            if not is_valid:
                ...
                prefixed_errors = [f"[Feuille '{sheet_name}'] {error}" for error in sheet_errors]
        ...
        if validation_errors and len(all_sheets_data) == 0:
            ...
            return jsonify({"error": "Validation échouée", "details": validation_errors}), 400
        ...
        if connection:
            cursor = connection.cursor()
            error_msg = None if not validation_errors else "; ".join(validation_errors[:5])  # Limiter les erreurs
            cursor.execute(
                """
                UPDATE file_history 
