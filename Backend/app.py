...
#     print("Nom de fichier personnalisé reçu :", custom_filename)
...
#         # Sauvegarder le fichier temporairement avec le nom original
#         upload_path = os.path.join(
#             app.config["UPLOAD_FOLDER"], f"{file_id}_{original_filename}"
#         )
#         file.save(upload_path)

#         # Enregistrer dans la base de données AVEC LE NOM ORIGINAL
...
#             """,
#                 (file_id, original_filename),  # Garder le nom original ici
#             )
#             connection.commit()
#             cursor.close()
#             connection.close()
...
#         is_valid, validation_errors = validate_excel_data(df)
...
#         # Préparer le nom du fichier traité
#         # Utiliser le nom personnalisé s'il est fourni, sinon utiliser l'original
...
#         # Créer le fichier Excel traité avec le nom personnalisé
#         processed_filename = base_filename
#         print("Nom du fichier traité :", processed_filename)
...
#         # Données
#         for row_num, record in enumerate(processed_data, 2):
...
#         # Mettre à jour le statut en succès AVEC LE NOM PERSONNALISÉ DANS processed_filename
#         connection = get_db_connection()
#         if connection:
#             cursor = connection.cursor()
#             cursor.execute(
#                 """
#                 UPDATE file_history
#                 SET status = 'success', processed_filename = %s, record_count = %s
#                 WHERE file_id = %s
#             """,
#                 (processed_filename, len(processed_data), file_id),
#             )
#             connection.commit()
#             cursor.close()
#             connection.close()
...
#                     "data": processed_data[:5],  # Retourner un aperçu
#                 }
#             ),
#             200,
#         )
...
# # Nouvelle route pour télécharger avec choix de format
...
#         # Chemin du fichier source (toujours en .xlsx)
#         source_path = os.path.join(
#             app.config["PROCESSED_FOLDER"], result["processed_filename"]
#         )
...
#         if format == "xlsx":
#             # Format par défaut - envoyer directement
#             cursor.close()
#             connection.close()
#             return send_file(
#                 source_path,
...
#         elif format == "xlsm":
#             # Sauvegarder en tant que macro-enabled
#             output = BytesIO()
#             wb.save(output)
#             output.seek(0)
#             cursor.close()
...
#         elif format == "xlsb":
#             # Note: openpyxl ne supporte pas directement xlsb
#             # On envoie en xlsx avec extension xlsb
#             output = BytesIO()
#             wb.save(output)
#             output.seek(0)
#             cursor.close()
...
#         elif format == "xls":
#             # Conversion en ancien format Excel
#             # On utilise openpyxl pour lire et on envoie en xlsx avec extension xls
#             # (xlwt a des limitations et compatibilité limitée)
#             output = BytesIO()
#             wb.save(output)
#             output.seek(0)
#             cursor.close()
...
#         elif format == "csv":
#             # Convertir en CSV (première feuille seulement)
...
#         elif format == "ods":
#             # Conversion en ODS (via pandas avec odfpy)
#             output = BytesIO()
...
#                 )
#             except Exception as e:
#                 # Si odf ne fonctionne pas, essayer avec xlsxwriter
